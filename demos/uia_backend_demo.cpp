...
{
    if (title == nullptr)       title = _T("Error");
    if (description == nullptr) description = _T("An unspecified problem occurred.");
...
static void ShowWin32Error(HWND owner, LPCTSTR whatFailed)
{
    ...
    const tstring what = (whatFailed != nullptr) ? tstring(whatFailed) : tstring(_T("A Win32 API call"));
    const tstring text = std::format(
        _T("{} failed.\n\nWin32 error {}: {}"),
        what, err, sysMsg);
...
static void RemoveNewlinesFromEdit(HWND hEdit)
{
    ...
    if (len < 0)
    {
        ShowProblem(GetParent(hEdit), _T("Error"), _T("GetWindowTextLength returned a negative length."));
        return;
    }
    if (len == 0) return;
    ...
    if (!buf)
    {
        ShowProblem(GetParent(hEdit), _T("Out of memory"), _T("Failed to allocate memory to sanitize pasted text."));
        return;
    }
    ...
    SendMessage(hEdit, EM_GETSEL, reinterpret_cast<WPARAM>(&selStart), reinterpret_cast<LPARAM>(&selEnd));
    int write = 0;
    ...
    if (write != got)
    {
        ...
        if (selStart > static_cast<DWORD>(write)) selStart = static_cast<DWORD>(write);
        if (selEnd   > static_cast<DWORD>(write)) selEnd   = static_cast<DWORD>(write);
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            g_hEdit = CreateWindowEx(
                ...
                0, 0, 0, 0,
                hwnd,
                reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_RICHEDIT)),
                g_hInstance,
                nullptr);
            if (!g_hEdit)
            {
            ...
            SetLastError(0);
            g_EditOrigProc = reinterpret_cast<WNDPROC>(
                SetWindowLongPtr(g_hEdit, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(EditSubclassProc)));
            if (!g_EditOrigProc)
            {
            ...
            g_hButton = CreateWindowEx(
                ...
                0, 0, 0, 0,
                hwnd,
                reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_SAY)),
                g_hInstance,
                nullptr);
            if (!g_hButton)
            {
        ...
        case WM_COMMAND:
        {
            ...
            {
            auto* backend = reinterpret_cast<PrismBackend*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
        if (!backend)
        {
            ShowProblem(hwnd, _T("Prism error"), _T("Backend pointer is null (window user data not set)."));
            return 0;
        }
        if (!g_hEdit)
        {
        ...
        LRESULT cch = SendMessage(g_hEdit, EM_GETTEXTLENGTHEX, reinterpret_cast<WPARAM>(&gtl), 0);
        if (cch < 0)
        {
            ShowProblem(hwnd, _T("Error"), _T("Failed to query RichEdit text length."));
            return 0;
        }
        std::wstring w;
        ...
        LRESULT copied = SendMessage(g_hEdit, EM_GETTEXTEX, reinterpret_cast<WPARAM>(&gte), reinterpret_cast<LPARAM>(w.data()));
        if (copied < 0)
        {
        ...
        int written = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, w.c_str(), -1, text.data(), needed, nullptr, nullptr);
        if (written == 0)
        {
...
if (const auto res = prism_backend_speak(backend, ctext, true); res != PRISM_OK)
{
    const char* err8 = prism_error_string(res);
    if (!err8) err8 = "unknown";
    int wneeded = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, err8, -1, nullptr, 0);
    if (wneeded == 0)
    {
        ShowWin32Error(hwnd, _T("MultiByteToWideChar(size query for prism_error_string)"));
        return 0;
    }
    std::wstring werr;
    werr.resize(static_cast<size_t>(wneeded));
    int wwritten = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, err8, -1, werr.data(), wneeded);
    if (wwritten == 0)
    {
        ShowWin32Error(hwnd, _T("MultiByteToWideChar(convert for prism_error_string)"));
        return 0;
    }
    if (!werr.empty() && werr.back() == L'\0')
        werr.pop_back();
#ifdef UNICODE
    const tstring msg = std::format(_T("Could not speak text to UIA: {}"), werr);
#else
    const tstring msg = std::format(_T("Could not speak text to UIA: {}"), err8);
#endif
    ...
    return 0;
        ...
        case WM_DESTROY:
        {
            ...
            {
                SetLastError(0);
                const LONG_PTR prev = SetWindowLongPtr(g_hEdit, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(g_EditOrigProc));
                if (prev == 0)
                {
                    const DWORD e = GetLastError();
                    if (e != 0)
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE, LPTSTR, int nCmdShow)
{
    ...
    if (!backend) {
    MessageBoxEx(hwnd, _T("Could not create UIA backend!"), _T("Prism error"), MB_ICONERROR | MB_OK, 0);
    prism_shutdown(ctx);
            DestroyWindow(hwnd); // triggers cleanup
        UnregisterClass(kClassName, hInstance);
        FreeLibrary(g_hRichDll);
