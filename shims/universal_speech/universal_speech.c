...
US_EXPORT(int) jfwIsAvailable(void) { return us_id_available(PRISM_BACKEND_JAWS); }
...
US_EXPORT(int) nvdaIsAvailable(void) { return us_id_available(PRISM_BACKEND_NVDA); }
...
US_EXPORT(int) weLoad(void) { return us_id_available(PRISM_BACKEND_WINDOW_EYES); }
...
US_EXPORT(int) weStopSpeech(void) { return us_id_stop(PRISM_BACKEND_WINDOW_EYES); }

US_EXPORT(int) saLoad(void) { return us_id_available(PRISM_BACKEND_SYSTEM_ACCESS); }
...
US_EXPORT(int) ztStopSpeech(void) { return us_id_stop(PRISM_BACKEND_ZOOM_TEXT); }
...
US_EXPORT(int) zdsrIsAvailable(void) { return us_id_available(PRISM_BACKEND_ZDSR); }
...
US_EXPORT(int) zdsrIsSpeaking(void) {
  fast_lock_acquire(&us_lock);
  PrismBackend *backend = us_backend_locked(us_index_for_id(PRISM_BACKEND_ZDSR));
  bool speaking = false;
  const int result =
...
US_EXPORT(int) narIsAvailable(void) { return us_id_available(PRISM_BACKEND_UIA); }
...
US_EXPORT(int) sapiSetOutputCallback(int sample_rate, UsWaveCallback callback,
                                 void *userdata) {
  fast_lock_acquire(&us_lock);
