...
#define DJINNI_SYSTEM_ASSERT(path, line, message)                              \
  __assert_rtn(reinterpret_cast<const char *>(-1L), path, line, message)
...
/*
 * Assert macro for expressions that don't contain JVM calls. Here we don't
 * check for exceptions thus saving some cycles.
 */
...
#define DJINNI_ASSERT_NO_EXC_CHECK(check)                                      \
  DJINNI_ASSERT_NO_EXC_CHECK_MSG(check, #check)
...
void onThreadExit(void *) { g_cachedJVM->DetachCurrentThread(); }
...
static auto &getMethodRecords() {
  static std::vector<std::tuple<const char *, const JNINativeMethod *, size_t>>
      methods;
  return methods;
}
...
void jniInit(JavaVM *jvm) {
  ...
  g_loadClassMethodID = env->GetMethodID(
  ...
  try {
    ...
    // Default exception handling only, since non-default might not be safe if
    // init is incomplete.
    jniDefaultSetPendingFromCurrent(jniGetThreadEnv(), __func__);
  }
}

void jniRegisterMethodRecords(const char *className,
                              const JNINativeMethod *records, size_t size) {
  getMethodRecords().emplace_back(className, records, size);
}

void jniRegisterNatives(JNIEnv *env, const char *className,
                        const JNINativeMethod *records, size_t size) {
  auto clazz = jniFindClass(className);
  env->RegisterNatives(clazz.get(), records, size);
}

void jniShutdown() { g_cachedJVM = nullptr; }

JNIEnv *jniGetThreadEnv() {
  assert(g_cachedJVM);
  JNIEnv *env = nullptr;
  jint get_res =
      g_cachedJVM->GetEnv(reinterpret_cast<void **>(&env), JNI_VERSION_1_6);
  if (get_res == JNI_EDETACHED) {
#if defined(ANDROID) || defined(__ANDROID__)
    get_res = g_cachedJVM->AttachCurrentThread(&env, nullptr);
#else
    get_res = g_cachedJVM->AttachCurrentThread(reinterpret_cast<void **>(&env),
                                               nullptr);
#endif
...
static JNIEnv *getOptThreadEnv() {
  ...
  // Special case: this allows us to ignore GlobalRef deletions that happen
  // after this thread has been detached. (This is known to happen during
  // process shutdown, when there's no need to release the ref anyway.)
  JNIEnv *env = nullptr;
  const jint get_res =
      g_cachedJVM->GetEnv(reinterpret_cast<void **>(&env), JNI_VERSION_1_6);
...
struct SystemClassInfo {
  ...
  const jmethodID staticmethIdentityHashCode{jniGetStaticMethodID(
      clazz.get(), "identityHashCode", "(Ljava/lang/Object;)I")};
};
} // namespace
...
size_t JavaIdentityHash::operator()(jobject obj) const {
  ...
  jint res = env->CallStaticIntMethod(sys.clazz.get(),
                                      sys.staticmethIdentityHashCode, obj);
  jniExceptionCheck(env);
  return res;
}
...
void jniThrowAssertionError(JNIEnv *env, const char *file, int line,
                            const char *check) {
  ...
  char buf[256];
  DJINNI_SNPRINTF(buf, sizeof buf, "djinni (%s:%d): %s", file_basename, line,
                  check);
...
GlobalRef<jclass> jniFindClass(const char *name) {
  ...
  if (!clazz) {
    ...
    clazz = static_cast<jclass>(
        env->CallObjectMethod(g_ourClassLoader, g_loadClassMethodID, jname));
    jniExceptionCheck(env);
  }
...
jmethodID jniGetStaticMethodID(jclass clazz, const char *name,
                               const char *sig) {
  JNIEnv *env = jniGetThreadEnv();
  ...
  jniExceptionCheck(env);
  if (!id) {
    jniThrowAssertionError(env, __FILE__, __LINE__,
                           "GetStaticMethodID returned null");
  }
  return id;
}
...
JniEnum::JniEnum(const std::string &name)
    : m_clazz{jniFindClass(name.c_str())},
      m_staticmethValues{jniGetStaticMethodID(m_clazz.get(), "values",
                                              ("()[L" + name + ";").c_str())},
...
LocalRef<jobject> JniEnum::create(JNIEnv *env, jint value) const {
  LocalRef<jobject> values(
      env, env->CallStaticObjectMethod(m_clazz.get(), m_staticmethValues));
  jniExceptionCheck(env);
  DJINNI_ASSERT_NO_EXC_CHECK(values);
  LocalRef<jobject> result(
      env, env->GetObjectArrayElement(static_cast<jobjectArray>(values.get()),
                                      value));
  jniExceptionCheck(env);
  return result;
}

JniFlags::JniFlags(const std::string &name) : JniEnum{name} {}
...
unsigned JniFlags::flags(JNIEnv *env, jobject obj) const {
  ...
  for (jint i = 0; i < size; ++i) {
    auto jf =
        LocalRef<jobject>(env, env->CallObjectMethod(it, m_iterator.methNext));
    jniExceptionCheck(env);
...
LocalRef<jobject> JniFlags::create(JNIEnv *env, unsigned flags,
                                   int bits) const {
  auto j = LocalRef<jobject>(
      env,
      env->CallStaticObjectMethod(m_clazz.get(), m_methNoneOf, enumClass()));
  jniExceptionCheck(env);
  unsigned mask = 1;
...
namespace {
[[noreturn]] inline void throwConversionError(const char *what,
                                              std::size_t position) {
  throw std::range_error(std::format(
      "{} (at input offset {})", std::string(what), std::to_string(position)));
}

inline constexpr jchar kEmptyJChars[1] = {0};
} // namespace

jstring jniStringFromWString(JNIEnv *env, const std::wstring &str) {
  if (str.empty()) [[unlikely]] {
    jstring res = env->NewString(kEmptyJChars, 0);
    DJINNI_ASSERT(res, env);
    return res;
  }
  std::u16string u16;
  if constexpr (sizeof(wchar_t) == sizeof(char16_t)) {
    const auto *src = reinterpret_cast<const char16_t *>(str.data());
    const auto vr = simdutf::validate_utf16le_with_errors(src, str.size());
    if (vr.error != simdutf::error_code::SUCCESS) [[unlikely]] {
      throwConversionError("jniStringFromWString: invalid UTF-16 in wstring",
                           vr.count);
    }
    u16.assign(src, str.size());
  } else {
    static_assert(sizeof(wchar_t) == sizeof(char32_t),
                  "wchar_t must be either 16 or 32 bits wide");
    const auto *src = reinterpret_cast<const char32_t *>(str.data());
    simdutf::result r{};
    u16.resize_and_overwrite(
        str.size() * 2,
        [&](char16_t *buf, [[maybe_unused]] std::size_t cap) noexcept {
          r = simdutf::convert_utf32_to_utf16le_with_errors(src, str.size(),
                                                            buf);
          return r.error == simdutf::error_code::SUCCESS ? r.count : 0;
        });
    if (r.error != simdutf::error_code::SUCCESS) [[unlikely]] {
      throwConversionError("jniStringFromWString: invalid UTF-32 in wstring",
                           r.count);
    }
  }
  jstring res = env->NewString(reinterpret_cast<const jchar *>(u16.data()),
...
std::wstring jniWStringFromString(JNIEnv *env, const jstring jstr) {
  ...
  const auto length = env->GetStringLength(jstr);
  if (length == 0) [[unlikely]] {
    return {};
  }
  const auto *u16 = env->GetStringChars(jstr, nullptr);
  DJINNI_ASSERT(u16, env);
  const auto *src = reinterpret_cast<const char16_t *>(u16);
  const auto srcLen = static_cast<std::size_t>(length);
  std::wstring out;
  try {
    if constexpr (sizeof(wchar_t) == sizeof(char16_t)) {
      const auto vr = simdutf::validate_utf16le_with_errors(src, srcLen);
      if (vr.error != simdutf::error_code::SUCCESS) [[unlikely]] {
        throwConversionError("jniWStringFromString: invalid UTF-16 in jstring",
                             vr.count);
      }
      out.assign(reinterpret_cast<const wchar_t *>(src), srcLen);
    } else {
      static_assert(sizeof(wchar_t) == sizeof(char32_t),
                    "wchar_t must be either 16 or 32 bits wide");
      simdutf::result r{};
      out.resize_and_overwrite(
          srcLen, [&](wchar_t *buf, [[maybe_unused]] std::size_t cap) noexcept {
            r = simdutf::convert_utf16le_to_utf32_with_errors(
                src, srcLen, reinterpret_cast<char32_t *>(buf));
            return r.error == simdutf::error_code::SUCCESS ? r.count : 0;
          });
      if (r.error != simdutf::error_code::SUCCESS) [[unlikely]] {
        throwConversionError("jniWStringFromString: invalid UTF-16 in jstring",
...
jstring jniStringFromUTF8(JNIEnv *env, const std::string &str) {
  ...
  u16.resize_and_overwrite(
      str.size(),
      [&](char16_t *buf, [[maybe_unused]] std::size_t cap) noexcept {
        r = simdutf::convert_utf8_to_utf16le_with_errors(str.data(), str.size(),
                                                         buf);
        return r.error == simdutf::error_code::SUCCESS ? r.count : 0;
      });
  if (r.error != simdutf::error_code::SUCCESS) [[unlikely]] {
    throwConversionError("jniStringFromUTF8: invalid UTF-8 input", r.count);
  }
  jstring res = env->NewString(reinterpret_cast<const jchar *>(u16.data()),
                               static_cast<jsize>(u16.size()));
  DJINNI_ASSERT(res, env);
...
std::string jniUTF8FromString(JNIEnv *env, const jstring jstr) {
  ...
  try {
    simdutf::result r{};
    out.resize_and_overwrite(
        srcLen * 3, [&](char *buf, [[maybe_unused]] std::size_t cap) noexcept {
          r = simdutf::convert_utf16le_to_utf8_with_errors(src, srcLen, buf);
          return r.error == simdutf::error_code::SUCCESS ? r.count
                                                         : std::size_t{0};
        });
    if (r.error != simdutf::error_code::SUCCESS) [[unlikely]] {
      throwConversionError("jniUTF8FromString: invalid UTF-16 in jstring",
                           r.count);
    }
  } catch (...) {
    env->ReleaseStringChars(jstr, u16);
...
void jniDefaultSetPendingFromCurrent(JNIEnv *env,
...
/*
 * Wrapper around Java WeakReference objects. (We can't use JNI
 * NewWeakGlobalRef() because it doesn't have the right semantics - see comment
 * in djinni_support.hpp.)
 */
class JavaWeakRef {
private:
  struct JniInfo {
  public:
    ...
    const jmethodID method_get{
        jniGetMethodID(clazz.get(), "get", "()Ljava/lang/Object;")};
  };
  ...
  static GlobalRef<jobject> create(JNIEnv *jniEnv, jobject obj) {
    ...
    LocalRef<jobject> weakRef(jniEnv,
                              jniEnv->NewObject(weakRefClass.clazz.get(),
                                                weakRefClass.constructor, obj));
    // DJINNI_ASSERT performs an exception check before anything else, so we
    // don't need a separate jniExceptionCheck call.
...
public:
  ...
  // Get the object pointed to if it's still strongly reachable or, return null
  // if not. (Analogous to weak_ptr::lock.) Returns a local reference.
  jobject lock() const {
  ...
  // Java WeakReference objects don't have a way to check whether they're
  // expired except by upgrading them to a strong ref.
  bool expired() const {
